...
export async function runSuiteCommand(command) {
  function parseCommand(command, type) {
    ...
    if (type === "connect") paramsString = command.slice(17, -1);
    else if (type === "disconnect") paramsString = command.slice(20, -1);
    else if (type === "sendRcv") paramsString = command.slice(15, -1);
    else if (type === "retDataToTables") paramsString = command.slice(23, -1);
    else if (type === "comparePairs") paramsString = command.slice(13, -1);
    ...
    if (command.startsWith("g40cli")) {
      if (command.includes("disconnectNE")) {
        ...
        const rb = {
          ...
          disconnect_status: "P",
          //connectRunStatus:connectStatus
        };

        let res = await axios.post(
          "http://35.188.41.6:8001/api/disconnectNE/",
          { ...rb }
        );
        sessionStorage.removeItem(data[0]);
        let isSuiteAborted = localStorage.getItem("isSuiteAborted");
        let pauseOnFail = localStorage.getItem("pauseOnFail");
        let abortOnFail = localStorage.getItem("abortOnFail");
        let singleStep = localStorage.getItem("singleStep");
        localStorage.clear();
        localStorage.setItem("isSuiteAborted",isSuiteAborted);
        ...
        let res = await axios.post(
          "http://35.188.41.6:8001/api/sendrcv/?Content-Type=application/json",
          { ...requestBody }
        );
        return res;
      }
    } else if (
      !command.startsWith("g40cli") &&
      command.includes("g40cli") &&
      command.includes("=") &&
      command.includes("sendRcv(")
    ) {
      ...
      command.includes("g40cli") &&
      command.includes("=") &&
      command.includes("retDataToTables(")
    ) {
      ...
      let res = await axios.post(
        "http://35.188.41.6:8001/api/retDataToTab/?Content-Type=application/json",
        { ...requestBody }
      // eslint-disable-next-line no-unused-vars
      ).catch(error => {
        localStorage.removeItem(arr[0].trim());
    });
      ...
      if (getFromLS) {
        ...
        let res = await axios.post(
          "http://35.188.41.6:8001/api/compare_pair/?Content-Type=application/json",
